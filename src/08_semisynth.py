...
def load(year):
    """Читает год. ИСХОДНЫЙ ФАЙЛ НЕ МЕНЯЕТСЯ — только чтение."""
    ...
    # РЕАЛЬНАЯ метка — сохраняется как есть
    y_real = pd.Series(np.select([oo <= 3, oo <= 15], [0, 1], default=2), index=df.index)
    out = df[keep].reset_index(drop=True)
    return out, y_real[keep].reset_index(drop=True).astype(int), oo[keep].reset_index(drop=True)
